  ...
  try {
    ...
    const { data: completedTickets, error: ticketsError } = await supabase
      ...
      .select(`
        ...
        related_company,
        tags,
        assigned_partner:saif_people!saif_tickets_assigned_to_fkey(id, first_name, last_name, email),
        company:saif_companies!saif_tickets_related_company_fkey(id, name),
        comments:saif_ticket_comments(id, content, is_final_comment, author:saif_people!saif_ticket_comments_author_id_fkey(first_name, last_name))
      `)
      .eq('status', 'archived')
